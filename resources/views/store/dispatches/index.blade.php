...
<html lang="en">
  <head>
    ...
    <title>Dispatches Log - Country Yoghurt MD</title>
    ...
    <div class="app-shell">
      ...
      <main class="main-content">
        <header class="topbar">
          ...
          <div class="top-actions">
            <a href="{{ route('store.dispatches.create') }}" class="primary-btn" style="text-decoration:none;">
              <i class="bi bi-plus-lg"></i> Record Dispatch
            </a>
          </div>
        </header>
        ...
        <section class="card inv-filter-bar">
          <form method="GET" action="{{ route('store.dispatches.index') }}" class="inv-filters">
            ...
            @if ($search)
              <a href="{{ route('store.dispatches.index') }}" class="ghost-btn">Clear</a>
            @endif
          </form>
                ...
                @empty
                  <tr>
                    <td colspan="7" class="empty-row">
                      <i class="bi bi-truck" style="font-size:1.5rem;display:block;margin-bottom:8px;color:#ccc;"></i>
                      No dispatches recorded. <a href="{{ route('store.dispatches.create') }}" class="link-btn">Record the first dispatch.</a>
                    </td>
                  </tr>
                @endforelse
              </tbody>
